...
use crate::{
    extensions::transparent::{Predicate, Witness},
...
mod context {
    ...
    impl<'a> V1<'a> {
        ...
        pub(super) fn is_tze_only(&self) -> bool {
        ...
        pub(super) fn tx_tze_outputs(&self) -> &[TzeOut] {
            &self.tx.tze_outputs
        }
    }
}
...
/// API for TZEs to access transaction and chain information.
///
...
impl<'a> Context<'a> {
    /// Generates a version 1 TZE context.
...
/// Enumeration of all transparent extensions to the Zcash consensus rules.
pub enum Programs {
    TestDummy,
}
